...
RUN apt-get install -y --no-install-recommends sudo php7.0 python3 python3-setuptools ca-certificates curl fakeroot gconf2 gconf-service git gvfs-bin libasound2 libcap2 libgconf-2-4 libgtk2.0-0 libgtk-3-0 libnotify4 libnss3 libxkbfile1 libxss1 libxtst6 libgl1-mesa-glx libgl1-mesa-dri xdg-utils libcanberra-gtk-module apt-utils apt-transport-https gnupg
...
RUN sudo apt-get install -y --no-install-recommends atom
